...
// VÉRIFIE SI LE SET ACTUEL CONTIENT DES CARTES VALIDES EN BASE
...
async function loadCards() {
    ...
    if (!error && data) {
        // Ne garder strictement que les cartes avec une image valide
...
async function openPack() {
    ...
    // Filtrer STRICTEMENT les cartes du set sélectionné
    const available = allDatabaseCards.value.filter(card => {
        if (!card || !card.set_id || !card.image_url) return false
        return String(card.set_id).replace('-', '').toLowerCase() === normalizedSelectedSet
    })

    // SI LE SET EST VIDE : ON STOPPE TOUT ET ON NE DÉDUIT AUCUNE GEMME !
    ...
    // Déduction des gemmes côté serveur uniquement si le set contient bien des cartes
...
function onImageError(event) {
    // Si une image échoue à charger, mettre une image de secours propre
...
.open-btn {
    ...
    color: #111827;
    font-size: 1rem;
    box-shadow: 0 6px 20px rgba(245, 158, 11, 0.24);
}
